...
function initHero(){const ss=document.querySelectorAll('.wf-s'),st=document.getElementById('wfSt');if(!ss.length)return;function run(){ss.forEach(s=>s.classList.remove('vis'));if(st){st.textContent='';st.className='wf-stat'}ss.forEach((s,i)=>{setTimeout(()=>{s.classList.add('vis');if(st){const tx=['Analyzing...','Responding...','Qualifying...','Ready for agent'],cl=['wf-stat','wf-stat','wf-stat','wf-stat ready'];st.textContent=tx[i]||'';st.className=cl[i]||'wf-stat'}},i*1400)});setTimeout(run,ss.length*1400+4000)}const ho=new IntersectionObserver(es=>{if(es[0].isIntersecting){run();ho.unobserve(es[0].target)}},{threshold:.2});ho.observe(document.getElementById('heroWf'))}